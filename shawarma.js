...
function showResult() {
  ...
  if (!anyChecked) {
    ...
    window.location.href =
      "ordersummary.html?result=" + encodeURIComponent(result);
  }
}
...
shawarmaOptions.forEach((option, index) => {
  ...
  shawarmaOptionContainer.appendChild(
    document.createTextNode(option.name + " - Price: " + option.price + "naira")
  );
...
function displayTime() { 
  let myReviewTime = document.getElementById("myReviewTime");
...
function updateMyReviewTime() {
  ...
  let timeDiff = Math.floor((presentTime - startTime) / 100);

  if (timeDiff < 10) {
    myReviewTime.textContent = "Just now";
  } else {
    let minutesAgo = Math.floor(timeDiff / 10); 
...
setInterval(updateMyReviewTime, 1000);
